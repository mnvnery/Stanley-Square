...
const ServicesIntro = ({image1, text1, image2, text2, pullOutQuote, quoteImage, title }) => {
    return (
        ...
        <div className={`bg-${color(title)} text-black pt-10 pb-16 z-[1] relative`}>
            ...
            <LeftMotion>
            <div className='flex flex-col-reverse pt-10 px-8 md:px-10 text-2xl xxl:text-5xl gap-8 md:gap-14 md:pt-20 md:grid grid-cols-[0.6fr_0.5fr] items-center xxl:gap-24'>
                <div className='relative w-100 h-[45vh] md:h-[76vh]'>
                    <Image src={quoteImage} objectFit='contain' layout='fill' className='rounded-xl'/>
                </div>
                <div dangerouslySetInnerHTML={{__html: pullOutQuote }} className='paragraph sale-about uppercase md:text-6xl md:mr-12 xxl:text-8xl'/>
            </div>
            </LeftMotion>
            <RightMotion>
            <div className='flex flex-col-reverse pt-10 px-8 md:px-10 text-2xl xxl:text-5xl gap-8 md:gap-14 md:pt-20 md:grid grid-cols-2 items-center'>
